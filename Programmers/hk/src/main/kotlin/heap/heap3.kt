...
/**
 * @desc
 * 이중 우선순위 큐는 다음 연산을 할 수 있는 자료 구조 입니다
 * 명령어	수신 탑(높이)
...
fun main() {
    val operations = arrayOf("I 16", "I 16", "D 1")
    println(solution(operations))
}
...
fun solution(operations: Array<String>): IntArray {
    ...
    for (i in operations) {
        val str = StringTokenizer(i, " ")
        val input = str.nextToken()
        val data = str.nextToken().toInt()
        ...
        if (input == "I") {
            ...
            if (minQueue.isNotEmpty() && maxQueue.isNotEmpty()) {
                if (data == -1) {
                    val minValue = minQueue.peek()
                    minQueue.poll()
                    maxQueue.remove(minValue)
                } else {
                    val maxValue = maxQueue.peek()
                    maxQueue.poll()
